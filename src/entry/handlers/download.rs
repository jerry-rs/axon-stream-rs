...
async fn download_file_stream(download_path:&std::path::Path, request: axum::http::Request<axum::body::Body>,)->axum::response::Response{
        ...
        .await
    {
        ...
        Err(e) => {
            ...
            ApiResponse::error(StatusCode::INTERNAL_SERVER_ERROR, "failed to read file").into_response()
        }
    }
}
            ...
            if file_type.is_dir() {
                ...
                if let Ok(target) = std::fs::read_link(&path) {
                    ...
                    if let Ok(metadata) = entry.metadata() {
                        let _ = header
                            .set_metadata_in_mode(&metadata, tar::HeaderMode::Deterministic);
                    }
